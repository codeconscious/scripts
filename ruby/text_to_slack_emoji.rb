...
if ARGV.count.zero? || ARGV.count > 2
  warn 'Pass in (1) a style name and (2) a string containing only supported characters for that style.'
  warn "Supported styles: #{supported_styles.join(', ')}"
  return
end
...
if ARGV.count == 1 && is_supported_style
  warn "The \"#{style}\" style supports the following characters:"
  warn style_chars(style, SPACE)
  return
end

unless is_supported_style
  warn "\"#{style}\" is not a supported style."
  warn "Supported styles: #{supported_styles.join(', ')}"
  return
end
...
if lowered_chars.empty?
  warn 'You must enter text for conversion.'
  return
end
...
unless lowered_chars.all?(&is_supported_char)
  warn "Only the following characters are supported for the \"#{style}\" style:"
  warn style_chars(style, SPACE)
  return
end
